...
### 新增
- 新增无 GUI Linux 服务器模式：提供 `pawzochat server init/run/doctor/passwd` 命令，使用固定单进程 HTTP 源站、外部可信 HTTPS Origin、独立 `/var/lib/pawzochat` 数据目录、持久化 Session/VAPID 身份、数据目录单实例锁和 systemd 部署模板；服务器无需桌面或浏览器，账号扫码与完整管理仍通过远程 Web 面板完成。
...
### 变更
- Web 认证由“是否经过第二个公网 Server”重构为明确访问策略：服务器入口始终要求管理员登录且允许已认证管理员管理 MCP/插件，部署参数只读、网页自更新关闭；桌面本地免登录与旧版随机路径公网入口保持兼容。
- 公网登录从“连续 5 次错误后全局锁死直到重启”改为按客户端 IP 的 15 分钟滑动窗口限流，避免攻击者用错误密码让其他设备无法登录；服务器密码命令会同步轮换 Session 密钥并撤销既有登录。
- 服务器进程不使用 Gunicorn 多 worker，继续复用 Cheroot 单进程线程池，防止消息队列、账号连接、主动消息和 Web Push 被多副本重复启动；端口占用时明确失败，不再终止占用端口的其他进程。

### 修复
- 修复依赖解析可能安装不兼容的 MCP 2.x、导致应用启动阶段导入失败的问题；当前客户端实现明确约束使用 MCP 1.x。
- Linux systemd 部署支持通过 `/etc/pawzochat/server.env` 为 PawzoChat 进程单独设置时区，避免服务器所在地时区影响消息时间、记忆与主动消息调度，且不修改宿主机时区。
